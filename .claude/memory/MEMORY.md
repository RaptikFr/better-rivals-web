...
- [Relais serveur & rang](relais-serveur-et-rang.md) — relais Python `relais_gui_v21.py` (GITIGNORÉ, fixe+portable) ; **dernière release v1.13.0 (trace télémétrie) publiée 19/06 depuis le PORTABLE (exe 14,5 Mo)** ; ✅ **divergence RÉSOLUE 20/06 : fixe + portable alignés sur v1.13.0** ; `gh` présent sur le fixe (release simple) / absent du portable (fallback Python/requests + git credential, sandbox désactivé)
