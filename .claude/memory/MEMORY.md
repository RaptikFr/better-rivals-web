...
- [Relais serveur & rang](relais-serveur-et-rang.md) — relais Python `relais_gui_v21.py` (GITIGNORÉ, fixe+portable) ; **dernière release v1.12.0 (secteurs) publiée 19/06 depuis le PORTABLE (exe 14,5 Mo)** ; ⚠️ **portable = copie à jour (v1.12.0), FIXE EN RETARD (v1.11.1)** → recopier avant d'éditer sur le fixe ; `gh` absent du portable → release via fallback Python/requests + git credential (sandbox désactivé)
