...
- [Relais serveur & rang](relais-serveur-et-rang.md) — relais Python `relais_gui_v21.py` (GITIGNORÉ) ; **release v1.11.2 publiée 19/06 (#13 check version, exe 14,5 Mo)** ; ⚠️ `gh` ABSENT du fixe → release via fallback Python/requests + git credential (sandbox désactivé) ; v1.11.0/v1.11.1 testées OK ; ⚠️ divergence source fixe/portable à surveiller
