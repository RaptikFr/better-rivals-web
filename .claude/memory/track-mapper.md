...
Écoute UDP FH6 port 5700 (même port que le relais), enregistre la trajectoire XZ d'un lap, marque des checkpoints au clavier [Espace], exporte :
...
|---|---|---|
| is_race_on | 0 | int32 |
| PositionX | 232 | float32 |
| PositionY | 236 | float32 |
| PositionZ | 240 | float32 |
| Speed | 256 | float32 |
...
Les offsets X/Y/Z (232/236/240) sont **à valider en jeu** — ce sont des offsets FH6-spécifiques (+12 vs Motorsport standard à cause de 3 champs FH6-only insérés après NumCylinders offset 228).
...
**Note** : le relais peut tourner en parallèle (même port UDP — Windows partage les sockets UDP en lecture). Mais vérifier que les deux n'entrent pas en conflit si le relais est en mode course.
