...
export default function DerniersChronos() {
  ...
  return (
    <div className="mt-24 max-w-5xl mx-auto w-full border-t border-neutral-800 pt-16">
      ...
      <p className="text-neutral-400 text-center mb-10">Les derniers temps enregistrés par la communauté.</p>

      <div className="flex flex-col gap-3">
        {chronos.map((lap) => (
          <div
            key={lap.id}
            className="bg-neutral-900 border border-neutral-800 rounded-xl px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6"
          >
            {/* Temps */}
            ...
            {/* Pilote + voiture + circuit */}
            <div className="flex-1 min-w-0">
              <p className="font-bold text-white truncate">{lap.players?.pseudo ?? '—'}</p>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className="px-2 py-0.5 bg-neutral-800 border border-neutral-700 rounded text-xs font-bold text-white">
                {lap.car_class}
              </span>
              {lap.drivetrain && (
                <span className={`px-2 py-0.5 border rounded text-xs font-bold ${DRIVETRAIN_COLORS[lap.drivetrain] ?? 'bg-neutral-800 border-neutral-700 text-neutral-400'}`}>
                  {lap.drivetrain}
                </span>
              )}
            </div>
      ...
      <div className="text-center mt-8">
        <Link
          href="/classements"
          className="inline-block px-6 py-3 bg-neutral-900 border border-neutral-700 text-white font-bold rounded-full hover:bg-neutral-800 transition-all"
        >
